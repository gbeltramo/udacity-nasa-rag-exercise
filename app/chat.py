...
import nasa_rag.llm_client as llm_client
import nasa_rag.rag_client as rag_client
import nasa_rag.ragas_evaluator as ragas_evaluator
...
def format_context(documents: List[str], metadatas: List[Dict], ids: List[str]) -> str:
    ...
    return rag_client.format_context(documents, metadatas, ids=ids)
...
def evaluate_response_quality(question: str, answer: str, contexts: List[str]) -> Dict[str, float]:
    ...
    try:
        return ragas_evaluator.evaluate_response_quality(question, answer, contexts, reference=None)
    except Exception as e:
...
def main():
    ...
    if "messages" not in st.session_state:
        # Note: create RAG specific system prompt
        system_prompt = """You are a NASA Subject Matter Expert assistant.
...
"""
    ...
    with st.sidebar:
        ...
        model_choice = st.selectbox(
            "OpenAI Model",
            options=["gpt-5-nano", "gpt-5-mini", "gpt-4.1-mini"],
            index=1,
